...
class Framework(object):
    ...
    def predict(self, data_loader: torch.utils.data.DataLoader) -> np.ndarray:
        ...
        for batch_x, _ in tqdm.tqdm(data_loader):
            current_probabilities = self.predict_probabilities(batch_x).astype(np.float32)#.detach().numpy()
...
class DropoutTrainer(Framework):
    def __init__(self, dataset_train,
        ...
        self.network2 = MNISTNet()
        self.network3 = MNISTNet()
        ...
        self.network.load_state_dict(torch.load('trainer_200.pth', map_location = torch.device('cpu')))
        self.network2.load_state_dict(torch.load('trainer_150.pth', map_location = torch.device('cpu')))
        self.network3.load_state_dict(torch.load('trainer.pth', map_location = torch.device('cpu')))
    ...
    def predict_probabilities(self, x: torch.Tensor, num_sample=100) -> torch.Tensor:
        ...
        #     preds.append(pred)
        with torch.no_grad():
                output1 = F.softmax(self.network(x), dim=1).cpu().numpy()
        with torch.no_grad():
                output2 = F.softmax(self.network2(x), dim=1).cpu().numpy()
        with torch.no_grad():
                output3 = F.softmax(self.network3(x), dim=1).cpu().numpy()
        # preds = np.array(preds)
        dropout_predictions = np.empty((0, x.shape[0], 10))
        for i in range(20):
        ...
        # estimated_probability = F.softmax(self.network(x), dim=1)
        # assert estimated_probability.shape == (x.shape[0], 10)
        estimated_probability = np.mean(dropout_predictions, axis=0) 

        dropout_predictions = np.empty((0, x.shape[0], 10))
        for i in range(20):
            predictions = np.empty((0, 10))
            enable_dropout(self.network2)
            with torch.no_grad():
                output = F.softmax(self.network2(x), dim=1)
            predictions = np.vstack((predictions, output.cpu().numpy()))
            dropout_predictions = np.vstack((dropout_predictions,
                                         predictions[np.newaxis, :, :]))
        # estimated_probability = F.softmax(self.network(x), dim=1)
        # assert estimated_probability.shape == (x.shape[0], 10)
        estimated_probability2 = np.mean(dropout_predictions, axis=0) 

        dropout_predictions = np.empty((0, x.shape[0], 10))
        for i in range(20):
            predictions = np.empty((0, 10))
            enable_dropout(self.network3)
            with torch.no_grad():
                output = F.softmax(self.network3(x), dim=1)
            predictions = np.vstack((predictions, output.cpu().numpy()))
            dropout_predictions = np.vstack((dropout_predictions,
                                         predictions[np.newaxis, :, :]))
        # estimated_probability = F.softmax(self.network(x), dim=1)
        # assert estimated_probability.shape == (x.shape[0], 10)
        estimated_probability3 = np.mean(dropout_predictions, axis=0) 



        return (estimated_probability + estimated_probability2 + estimated_probability3) / 3.0
